...
For our purposes, let's assume that you're using L<mySQL|http://www.mysql.com/>
...
to create a new user, it might be in Pizzaria::Web::Form::User::Register, or
...
repitition.  We already said that the two main functions we want users to
...
Let's create a form for creating new users.
	...
	$ touch ./lib/Pizzaria/Web/Form/User/Create.pm
...
L<this excellent introduction to HTML::FormHandler|http://www.catalyzed.org/2010/03/an-introduction-to-formhandler.html>
...
Then in our template (C<./root/src/register/index.tt>):
...
rendered through L<HTML::FormHandler> and L<The Template Toolkit|http://template-toolkit.org/>
to make AJAX calls?

Thankfully, someone who calls himself I<malsup> thought of that and gave us a
terrific solution:  L<the ajaxForm plugin|http://jquery.malsup.com/form/>!
...
form and the DB.  After that, we'll use jQuery UI's core plugin
L<Datepicker|http://jqueryui.com/demos/datepicker/> to do the heavy lifting.
...
customization.  Check out L<the demo|http://jqueryui.com/demos/datepicker/>
...
But no!  Check out the venerable L<UI.Layout plugin|http://layout.jquery-dev.net/>
...
=item	Check out L<the jQuery API|http://api.jquery.com/>

=item Peruse L<the jQuery UI demos|http://jqueryui.com/>
...
set of tutorials.  If I do, you'll be able to find them at L<my blog|http://snugglepunk.blogspot.com>.
